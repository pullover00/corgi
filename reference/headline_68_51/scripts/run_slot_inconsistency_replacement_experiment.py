...
from goldilocs.cache import load_reconstruction
from goldilocs.changesim import MetricAccumulator, normalize_target
from goldilocs.config import load_config
from goldilocs.experiments.branch_b2 import ConsolidationSettings, consolidate_hypotheses
from goldilocs.experiments.sam3_pairwise import load_cached_inputs, load_proposal_cache, proposals_to_objects
...
from goldilocs.geometry import render_points
from goldilocs.ground_contact import detect_floor_plane, up_vector_from_world_to_camera
from goldilocs.io import save_image, save_json

sys.path.insert(0, str(Path(__file__).resolve().parent))
from run_branch_b2_diagnostic import _load_feature_maps, _manifest_targets, _parent_records, _resolve  # noqa: E402
from run_sam3_moved_association_experiment import load_tracking_cache, recover_changed_candidates  # noqa: E402


REPOSITORY = Path(__file__).resolve().parents[1]
DEFAULT_CONFIG = REPOSITORY / "configs/experiments/changesim-branch-b2-consolidated-reciprocal-densegrid96.yaml"
DEFAULT_OUTPUT = REPOSITORY / "outputs/experiment-slot-inconsistency-replacement-densegrid96"
...
def _inference_pair(
    ...
    source_sam, target_sam = _load_feature_maps(_resolve(split["feature_cache_root"]), pair_id)
    source_dino, target_dino = _load_feature_maps(_resolve(split["dino_feature_cache_root"]), pair_id)
    ...
    trusted_targets = [targets[row["target_index"]].mask for row in cleanup_rows]
    ...
    consensus_baseline, consensus_changed, consensus_summary = dominant_changed_class_consensus(
        baseline,
        [targets[row["target_index"]].mask for row in plausibility_rejected_rows],
        minimum_changed_fraction=settings.consensus_minimum_changed_fraction,
        minimum_dominant_fraction=settings.consensus_minimum_dominant_fraction,
        minimum_pixels=settings.consensus_minimum_pixels,
    )
    replacement_ownership, depth_ownership = _replacement_depth_ownership(
        artifact,
        consensus_baseline,
        promoted_targets,
        settings,
    ...
    inference = {
        ...
        "cleanup_eligible_target_count": len(cleanup_rows),
...
def _html(output: Path, report: dict[str, Any]) -> None:
    ...
    if "overall" in report:
        ...
        for label, metrics in (
            ("Final R4 baseline", base),
            ("Depth-owned full target masks", full),
            ("Depth-owned guarded output", guarded),
        ):
        ...
        for family, title in (("binary", "Binary classes"), ("multiclass", "Multiclass classes")):
            ...
            class_sections.append(
                f"<h3>{title}</h3><table><tr><th>class</th><th>R4 IoU</th>"
        ...
        overall_html = f"""
        <section class='method'><h2>Combined 25-pair result</h2>
        <p>Pixel-count aggregate across fixed10 and new15. The baseline is the frozen
        densegrid96 real-image association resolver variant <code>r4_no_geometry_ablation</code>,
        not the earlier SAM2 tracking-v4 evidence experiment.</p>
        ...
        <p>The full-mask ablation has the higher binary mIoU. The guarded output changes
        fewer baseline pixels. Both variants enforce one front-most owner at overlaps.
        WARPED is not a scored ChangeSim ground-truth class.</p>
        <table class='summary'><tr><th>metric</th><th>R4 baseline</th><th>full target mask</th><th>guarded/trusted output</th><th>guarded delta</th></tr>
    ...
    for split_name, split in report["splits"].items():
        ...
        for pair in split["pairs"]:
            ...
            for item in pair["replacement_candidate_truth"]:
                ...
                cleanup_text = "allowed" if cleanup.get("eligible") else "blocked"
                cleanup_reasons = ", ".join(cleanup.get("reasons", [])) or "all safety checks passed"
            ...
            cards.append(f"""<article><h3>{html.escape(pair['id'])}</h3><p>slot matches {inf['slot_matches']} · replacement candidates {inf['replacement_candidate_count']} · plausible objects {inf['promoted_target_count']} · rejected fragments {inf['plausibility_rejected_count']} · suppressed floor pixels {inf['guarded_floor_suppressed_pixels']} · dropped old REMOVED pixels {inf['dropped_removed_pixels']} · full/guarded changed pixels {inf['full_prediction_changed_pixels']}/{inf['prediction_changed_pixels']} · guarded ΔmIoU {pair['delta']['multiclass_miou']:+.5f}</p><div class='images'><figure><img src='{root}/target.png'><figcaption>Target image</figcaption></figure><figure><img src='{root}/replacement_candidate_overlay.png'><figcaption>Raw identity-mismatch candidates</figcaption></figure><figure><img src='{root}/replacement_overlay.png'><figcaption>Plausible coherent SAM objects</figcaption></figure><figure><img src='{root}/replacement_depth_owned_overlay.png'><figcaption>Final object ownership</figcaption></figure><figure><img src='{root}/floor_suppression_overlay.png'><figcaption>Implausible floor component suppression</figcaption></figure><figure><img src='{root}/baseline_labels_color.png'><figcaption>R4 baseline</figcaption></figure><figure><img src='{root}/labels_full_mask_color.png'><figcaption>Object-consistent full output</figcaption></figure><figure><img src='{root}/labels_color.png'><figcaption>Object-consistent guarded output</figcaption></figure><figure><img src='{root}/ground_truth_color.png'><figcaption>Ground truth</figcaption></figure></div>{''.join(cases)}<details><summary>All slot decisions and raw metrics</summary><pre>{html.escape(json.dumps({'decisions': inf['decisions'], 'replacement_candidates': inf['replacement_candidates'], 'depth_ownership': depth, 'floor_suppression': floor}, indent=2))}</pre></details></article>""")
        representative = max(
            split["pairs"],
            key=lambda pair: (
        ...
        sections.append(f"""
        <h2>{split_name}</h2>
        <p>The full-mask variant labels the depth-owned part of each complete target SAM hypothesis. At every overlap, robust target-camera depth selects one object owner; ties preserve the existing R4 class. The more conservative guarded variant additionally requires R4 change support and only clears an old REMOVED footprint for strong one-to-one replacements.</p>
        <table class='summary'><tr><th>metric</th><th>R4 baseline</th><th>full target mask</th><th>guarded changed-pixel relabel</th><th>guarded delta</th></tr><tr><td>multiclass mIoU</td><td>{base['multiclass_miou']:.6f}</td><td>{full['multiclass_miou']:.6f}</td><td>{candidate['multiclass_miou']:.6f}</td><td>{delta['multiclass_miou']:+.6f}</td></tr><tr><td>binary mIoU</td><td>{base['binary_miou']:.6f}</td><td>{full['binary_miou']:.6f}</td><td>{candidate['binary_miou']:.6f}</td><td>{delta['binary_miou']:+.6f}</td></tr><tr><td>REPLACED IoU</td><td>{base['multiclass']['replaced']['iou']:.6f}</td><td>{full['multiclass']['replaced']['iou']:.6f}</td><td>{candidate['multiclass']['replaced']['iou']:.6f}</td><td>{delta['replaced_iou']:+.6f}</td></tr><tr><td>REPLACED precision</td><td>{base['multiclass']['replaced']['precision']:.6f}</td><td>{full['multiclass']['replaced']['precision']:.6f}</td><td>{candidate['multiclass']['replaced']['precision']:.6f}</td><td>{delta['replaced_precision']:+.6f}</td></tr><tr><td>REPLACED recall</td><td>{base['multiclass']['replaced']['recall']:.6f}</td><td>{full['multiclass']['replaced']['recall']:.6f}</td><td>{candidate['multiclass']['replaced']['recall']:.6f}</td><td>{delta['replaced_recall']:+.6f}</td></tr></table>
        ...
        <div class='pipeline'>
          ...
          <figure><span>4</span><img src='{representative_root}/replacement_overlay.png'><figcaption>Compare target SAM objects</figcaption></figure><b>→</b>
          <figure><span>5</span><img src='{representative_root}/depth_ownership_color.png'><figcaption>Assign each overlap to the nearer object</figcaption></figure><b>→</b>
          <figure><span>6</span><img src='{representative_root}/baseline_labels_color.png'><figcaption>Preserve front-most R4 owners</figcaption></figure><b>→</b>
          <figure><span>7</span><img src='{representative_root}/labels_color.png'><figcaption>Write only depth-owned REPLACED pixels</figcaption></figure>
        </div>
    ...
    body{{font:15px system-ui;max-width:1500px;margin:28px auto;background:#f3f6fa;color:#16202c;line-height:1.45}}article,.method{{background:white;border-radius:11px;padding:18px;margin:18px 0;box-shadow:0 2px 10px #20304014}}table{{border-collapse:collapse;width:100%;background:white;margin:10px 0 18px}}th,td{{padding:7px;border:1px solid #d8dfe8;text-align:left;vertical-align:top}}.summary th{{background:#203d5e;color:white}}.images{{display:grid;grid-template-columns:repeat(4,1fr);gap:9px}}.zoom-images{{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px;align-items:end}}figure{{margin:0}}img{{width:100%;display:block;background:#111;image-rendering:auto}}figcaption{{font-weight:600;padding-top:4px}}pre{{font-size:11px;white-space:pre-wrap}}.case{{border-top:3px solid #d9e1eb;margin-top:18px;padding-top:8px}}.verdict{{background:#f1f4f8;padding:7px}}.quiet{{color:#5b6977}}.pipeline{{display:grid;grid-template-columns:1fr auto 1fr auto 1fr auto 1fr auto 1fr auto 1fr auto 1fr;gap:8px;align-items:center;background:#e7edf4;padding:12px;border-radius:9px}}.pipeline b{{font-size:24px;color:#48627e}}.pipeline span{{position:absolute;background:#203d5e;color:white;border-radius:50%;padding:3px 9px;margin:5px}}.legend span{{display:inline-block;padding:4px 9px;border-radius:5px;margin-right:6px;color:white}}@media(max-width:1000px){{.images,.zoom-images{{grid-template-columns:1fr 1fr}}.pipeline{{grid-template-columns:1fr}}.pipeline b{{display:none}}}}</style></head><body>
    <h1>Object-slot inconsistency replacement experiment</h1>
    <div class='method'><p><b>Question:</b> can replacement be found by looking for an old object whose identity disappears in the same aligned slot, without producing a new clean render?</p><p><b>Corrected baseline:</b> all label edits are applied on top of the frozen densegrid96 association-resolver <code>r4_no_geometry_ablation</code> predictions. Tracking-v4 artifacts supply evidence only.</p><p><b>Single-owner depth rule:</b> every promoted target object receives a robust target-camera depth. Existing REMOVED components use source geometry rendered into the target camera; other changed components use target depth. Only the nearer object owns an overlap, and depth ties or missing depth preserve the existing R4 class.</p><p><b>Decision:</b> reject support-surface size mismatches and weak slots first. Route a strong propagated-track plus color match as motion. Otherwise require loss of aligned DINO patch identity and at least two mismatches among SAM identity, DINO identity, and color. Strong same-identity evidence rescues the object. Old REMOVED pixels are erased only after stricter one-to-one cleanup checks; the new target object is written only inside its depth-owned region.</p><p><b>Evaluation discipline:</b> predictions and SHA-256 hashes were frozen before ground truth was opened. Ground truth is used only below for scoring and failure analysis.</p><p class='legend'><span style='background:#1ed250'>ADDED</span><span style='background:#eb4632'>REMOVED</span><span style='background:#ffa514'>MOVED</span><span style='background:#1ea0e6'>WARPED</span><span style='background:#cd3ceb'>REPLACED</span></p></div>
...
def main() -> None:
    ...
    for split_name, split in config["splits"].items():
        ...
        selection, records = _parent_records(_resolve(split["parent_evaluation"])); targets = _manifest_targets(_resolve(split["manifest"]))
