...
def frontmost_replacement_ownership(
    ...
    *,
    ...
    ownership = candidate_union & candidate_known & (
        ~existing_changed | already_replaced | candidate_front
    )
    # Once the plausibility gate has established that a SAM hypothesis is one
    # physical replacement object, R4 class fragments inside that proposal are
    # evidence about the same object, not independent occluders. Give the whole
    # visible proposal one class. Depth remains the validity/ordering signal
    # among target object proposals rather than a source of class stripes.
    ownership |= unified_objects & candidate_known
    ...
    summary = {
        ...
        "frontmost_promoted_overlap_pixels": int(promoted_overlap.sum()),
...
def rasterize_replacement_labels(
    ...
    *,
    ...
    dropped_removed = old_mask & ~new_mask & (labels == removed_label)
...
def replacement_cleanup_evidence(
    ...
    *,
    ...
    """Decide whether a replacement is strong enough to erase its old footprint."""
    ...
    reasons = []
    if not identity_ok:
        reasons.append("insufficient_identity_mismatches")
    if not one_to_one and not compact_foreground:
        reasons.append("neither_one_to_one_nor_compact_foreground")
    if not semantic_ok:
