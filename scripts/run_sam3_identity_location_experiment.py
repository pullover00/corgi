#!/usr/bin/env python3
"""Run the isolated SAM3 identity + aligned-location ten-pair experiment."""
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "configs/stages/changesim-sam3-identity-location-no-splat.yaml"
        ),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/benchmark-sam3-identity-location-no-splat"),
    )
...
def main() -> int:
    ...
    sam3 = config["sam3"]
    source_path = Path(sam3["source"]).resolve()
    checkpoint = Path(sam3["checkpoint"]).resolve()
    if _git_commit(source_path) != sam3["source_commit"]:
        raise RuntimeError("SAM3 source commit differs from the frozen config")
    checkpoint_hash = _sha256(checkpoint)
    if checkpoint_hash != sam3["checkpoint_sha256"]:
        raise RuntimeError("SAM3 checkpoint SHA-256 differs from the frozen config")
    ...
    execution_hash = _json_hash(
        {
            ...
            "sam3_checkpoint_sha256": checkpoint_hash,
        }
    )
        ...
        validation_records.append(
            {
                "id": pair_id,
                "pair_fingerprint_sha256": fingerprint,
                "source_visible": len(context["source_objects"]),
                "target_visible": len(context["target_objects"]),
            }
        )
        print(
    ...
    if missing_features:
        extractor = Sam3FeatureExtractor(source_path, checkpoint)
        try:
            for number, pair_id in enumerate(missing_features, 1):
                started = time.perf_counter()
                # Contexts contain full-resolution masks and images. Rebuild
                # one at a time instead of retaining a shard's worth in RAM.
                context = _pair_inputs(config, parent_records[pair_id])
    ...
    for number, pair_id in enumerate(selected_ids, 1):
        ...
        try:
            ...
            source_descriptors = mask_descriptors(
                ...
                minimum_feature_cells=float(sam3["minimum_feature_cells"]),
            )
    ...
    report = {
        ...
        "provenance": {
            ...
            "execution_sha256": execution_hash,
            "parent_report_sha256": parent_hash,
            "sam3_source_commit": sam3["source_commit"],
            "sam3_checkpoint_sha256": checkpoint_hash,
            "ground_truth_used_in_inference": False,
            "tuned_on_test": False,
        },
