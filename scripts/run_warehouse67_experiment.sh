...
# Warehouse_6 and Warehouse_7 pair in ChangeSim (2,122 pairs total: 1,100 +
# 1,022), for the paper's main quantitative evaluation, qualitative figures,
...
#   ./scripts/run_warehouse67_experiment.sh
#
...
OUTPUT_DIR="${OCMASK_WAREHOUSE67_OUTPUT:-$REPO/out/warehouse67_experiment}"
MANIFEST="$REPO/data/changesim/manifest-warehouse67.jsonl"
...
# operator's own environment always wins. See README.md's "Environment
# variables" section for what these must point at.
...
if [[ ! -f "$MANIFEST" ]]; then
    ...
    echo "Regenerate it from data/changesim/manifest-table3.jsonl (filter id startswith Warehouse_6_/Warehouse_7_)." >&2
    exit 1
fi

PAIR_COUNT="$(wc -l < "$MANIFEST" | tr -d ' ')"
echo "=== Warehouse_6 + Warehouse_7 full-pipeline experiment ==="
