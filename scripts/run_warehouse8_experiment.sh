...
# Warehouse_8 pair in ChangeSim (2,373 pairs) -- split out of
# scripts/run_warehouse89_experiment.sh so Warehouse_8 can start on the
# second machine as soon as its data finishes syncing, independently of
# Warehouse_9 (synced separately, run via scripts/run_warehouse9_experiment.sh
# once its data lands).
#
...
#   ./scripts/run_warehouse8_experiment.sh
#
...
OUTPUT_DIR="${OCMASK_WAREHOUSE8_OUTPUT:-$REPO/out/warehouse8_experiment}"
MANIFEST="$REPO/data/changesim/manifest-warehouse8.jsonl"
...
if [[ ! -f "$MANIFEST" ]]; then
    ...
    echo "Regenerate it from data/changesim/manifest-warehouse89.jsonl (filter id startswith Warehouse_8_)." >&2
    exit 1
fi

PAIR_COUNT="$(wc -l < "$MANIFEST" | tr -d ' ')"
echo "=== Warehouse_8 full-pipeline experiment ==="
